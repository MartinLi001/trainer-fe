...
import { SearchModelState, connect, Dispatch } from 'umi';
...
const Question: React.FC<QuestionProps> = ({ moduleSearchList, dispatch }) => {
  ...
  const deleteQuestion = (id: string) => {
    Modal.confirm({
      ...
      content: (
        <div>
          <p style={{ fontSize: 16, fontWeight: 600, lineHeight: '26px' }}>
            Are you sure of deleting the question?
          </p>
      ...
      onOk() {
        removeQuesiton({ questionId: id }).then(() => {
  ...
  const editQuestion = (id: string) => {
    history.push(`/question/questionadd/${id}`);
  };

  const renderMenu = (item: SearchData) => {
    const menuItems = [
      {
        label: <a onClick={() => editQuestion(item.questionId)}>edit</a>,
        key: 'edit',
      },
      {
        label: <a onClick={() => deleteQuestion(item.questionId)}>delete</a>,
        key: 'delete',
      },
    ];
  ...
  const columns: ColumnsType<SearchData> = [
    ...
    },
    {
      dataIndex: '',
      render: (text, i) => (
        <Dropdown overlay={renderMenu(i)} trigger={['click']}>
  ...
  const addNewQuestion = () => {
    history.push(`/question/questionadd/newADD`);
  };
  const renderClear = () => {
    if (
  ...
  return (
    <>
      <div className="QuestionTitle">
        <PageHeader items={[{ name: 'Question Bank' }]} />
        {!showPage && (
          <div className="QuestionList-Search">
      ...
      <Spin spinning={pageLoading} size="large">
        <div className="QuestionList">
          <div className="QuestionList-Head">
          ...
          <div className="QuestionList-Tag">
            <div className="QuestionList-Tag-Search">
              <DropDownTag
                list={[
                  {
                    label: 'Tags',
            ...
            <div className="QuestionList-Tag-Button">
              <SeeButton
                className="QuestionList-Tag-add"
                icon={<PlusOutlined />}
                type="primary"
